...
function saveToStorage(providers: ProviderWithStatus[]) {
  // Only save the locally-submitted entries (id starts with "local-")
  const local = providers.filter((p) => p.id.startsWith("local-"));
  localStorage.setItem("arubaProApplications", JSON.stringify(local));
}
...
export default function AdminPage() {
  ...
  // Merge localStorage submissions on mount
  useEffect(() => {
    const stored: ProviderWithStatus[] = JSON.parse(
      localStorage.getItem("arubaProApplications") || "[]"
    );
    if (stored.length > 0) {
      setProviders((ps) => {
        const existingIds = new Set(ps.map((p) => p.id));
        const fresh = stored.filter((p) => !existingIds.has(p.id));
        return [...ps, ...fresh];
      });
    }
  }, []);
  ...
  const avgRating = approved.reduce((sum, p) => sum + (p.rating ?? 0), 0) / (approved.filter((p) => p.rating).length || 1);

  function approve(id: string) {
    setProviders((ps) => {
      const next = ps.map((p) => (p.id === id ? { ...p, status: "approved" as const } : p));
      saveToStorage(next);
      return next;
    });
  }
  ...
  function reject(id: string) {
    setProviders((ps) => {
      const next = ps.map((p) => (p.id === id ? { ...p, status: "rejected" as const } : p));
      saveToStorage(next);
      return next;
    });
  }
      ...
      <main className="flex-1 pt-16 bg-slate-50">
        ...
        <div className="bg-navy-900 py-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <span className="inline-flex items-center gap-1.5 text-xs bg-amber-500/20 text-amber-300 border border-amber-500/30 px-3 py-1.5 rounded-full font-medium">
              <Clock className="w-3.5 h-3.5" /> New applications appear here automatically
            </span>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={<Users className="w-5 h-5 text-brand-500" />} label="Total Providers" value={providers.length} />
            <StatCard icon={<Clock className="w-5 h-5 text-amber-500" />} label="Pending Review" value={pending.length} highlight={pending.length > 0} />
            <StatCard icon={<CheckCircle className="w-5 h-5 text-green-500" />} label="Approved" value={approved.length} />
            <StatCard icon={<Star className="w-5 h-5 text-amber-400" />} label="Avg. Rating" value={avgRating.toFixed(1)} />
          </div>
                          ...
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3">
                              {p.imageUrl ? (
                                ...
                                <div className="w-9 h-9 bg-slate-200 rounded-lg flex items-center justify-center text-xs font-bold text-slate-600 flex-shrink-0">
                                  {p.initials}
                                </div>
                              )}
                              <div>
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="inline-flex items-center justify-center w-6 h-6 bg-green-500 text-white text-xs font-bold rounded-full">{approved.length}</span>
            </div>
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                      ...
                      <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            {p.imageUrl ? (
                              ...
                              <div className="w-9 h-9 bg-navy-900 rounded-lg flex items-center justify-center text-xs font-bold text-brand-400 flex-shrink-0">
                                {p.initials}
                              </div>
                            )}
                            <div>
                        ...
                        <td className="px-5 py-4">
                          <button onClick={() => remove(p.id)}
                            className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Remove listing">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100">
                        <Th>Business</Th><Th>Category</Th><Th>District</Th><Th>Actions</Th>
                      </tr>
                    </thead>
                    <tbody>
...
function Th({ children }: { children: React.ReactNode }) {
  return <th className="px-5 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">{children}</th>;
}
